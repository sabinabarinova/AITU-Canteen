                            ...
                            <tbody>
                                <?php
                                    ...
                                    while($myrow = mysqli_fetch_assoc($myresult)){
                                        ...
                                        $itemsql = "SELECT * FROM `products` WHERE prodID = $prodID";
                                        $itemresult = mysqli_query($conn, $itemsql);
                                        $itemrow = mysqli_fetch_assoc($itemresult);
                                        $prodName = $itemrow['prodName'];
                                        $prodPrice = $itemrow['prodPrice'];
                                        $prodDesc = $itemrow['prodDesc'];
                                        $prodCategorieId = $itemrow['prodCategorieId'];

                                        echo '<tr>
                                                <th scope="row">
                                                    <div class="p-2">
                                                    <img src="img/pizza-'.$prodID. '.jpg" alt="" width="70" class="img-fluid rounded shadow-sm">
                                                    <div class="ml-3 d-inline-block align-middle">
                                                        <h5 class="mb-0"> <a href="#" class="text-dark d-inline-block align-middle">'.$prodName. '</a></h5><span class="text-muted font-weight-normal font-italic d-block">KZT ' .$prodPrice. '</span>
                                                    </div>
                                                    </div>
                                                </th>
